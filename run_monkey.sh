...
if [[ $MONKEY_SEED -eq 0 ]]
then
    timeout $TEST_TIME adb -s $AVD_SERIAL shell monkey -p $app_package_name -v --throttle 200 --pct-syskeys 0 --ignore-crashes --ignore-timeouts --ignore-security-exceptions --bugreport 1000000 2>&1 | tee $result_dir/monkey.log
else
    timeout $TEST_TIME adb -s $AVD_SERIAL shell monkey -s $MONKEY_SEED -p $app_package_name -v --throttle 200 --pct-syskeys 0 --ignore-crashes --ignore-timeouts --ignore-security-exceptions --bugreport 1000000 2>&1 | tee $result_dir/monkey.log
fi
